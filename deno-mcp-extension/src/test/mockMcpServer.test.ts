...
suite("Mock MCP Server Tests", () => {
  suite("Mock Server Structure", () => {
    test("Should have basic MCP server interface", () => {
      ...
      assert.strictEqual(mockServer.version, "1.0.0", "Should have version");
      assert.ok(Array.isArray(mockServer.tools), "Should have tools array");
      assert.ok(mockServer.tools.length > 0, "Should have at least one tool");
    });

    test("Should provide expected tools", () => {
      const expectedTools = ["format", "lint", "check", "test", "info", "run"];
      ...
      for (const toolName of expectedTools) {
        ...
        assert.ok(toolName.length > 0, `Tool ${toolName} should not be empty`);
      }
    });
  });
  ...
  suite("Mock Tool Responses", () => {
    ...
    test("Should simulate lint tool response", () => {
      ...
      assert.ok(mockResponse.success, "Should indicate success");
      assert.ok(Array.isArray(mockResponse.issues), "Should have issues array");
    });
  ...
  suite("Mock Server Behavior", () => {
    ...
    test("Should handle invalid tool requests", () => {
      ...
      const validTools = ["format", "lint", "check", "test", "info", "run"];
    ...
    test("Should provide tool descriptions", () => {
      ...
      for (const [tool, description] of Object.entries(toolDescriptions)) {
        assert.ok(typeof tool === "string", "Tool name should be string");
        assert.ok(
  ...
  suite("MCP Protocol Compliance", () => {
    test("Should follow MCP tool response format", () => {
      ...
      assert.strictEqual(mcpResponse.jsonrpc, "2.0", "Should use JSON-RPC 2.0");
    ...
    test("Should support transport modes", () => {
      ...
      for (const transport of supportedTransports) {
        assert.ok(typeof transport === "string", "Transport should be string");
        assert.ok(
