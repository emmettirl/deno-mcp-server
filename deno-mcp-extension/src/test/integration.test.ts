...
suite("Integration Tests", () => {
  ...
  suite("Configuration Integration", () => {
    test("Should integrate with VS Code settings", () => {
      ...
      assert.ok(
        port !== undefined,
        "Port configuration should be accessible",
      );
      assert.ok(
    ...
    test("Should handle document save events", () => {
      ...
      const disposable = vscode.workspace.onDidSaveTextDocument(
        (document) => {
          ...
          assert.ok(
            document,
            "Document should be provided in save event",
          );
        },
      );
      ...
      assert.ok(
        disposable,
        "Should be able to register save event listener",
      );
      disposable.dispose();
    });
      ...
      const disposable = vscode.workspace.onDidChangeConfiguration(
        (event) => {
          if (event.affectsConfiguration("deno-mcp")) {
            assert.ok(
              true,
              "Should detect Deno MCP configuration changes",
            );
          }
        },
      );
