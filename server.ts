...
export class DenoMCPServer {
  ...
  private handlers: Map<string, (params: ToolArgs) => Promise<Record<string, unknown>>> = new Map();
    ...
    try {
      switch (request.method) {
        ...
        case "tools/call": {
          ...
          const result = await handler((request.params?.arguments || {}) as ToolArgs);
          return {
            jsonrpc: "2.0",
            id: request.id,
            result
  ...
  private async executeDeno(args: string[], cwd?: string): Promise<{ 
    ...
    try {
      ...
      return {
        stdout: "",
        stderr: `Error executing deno command: ${error instanceof Error ? error.message : String(error)}`,
        success: false,
        code: -1
      };
    }
  ...
  private async handleDenoFmt(args: ToolArgs): Promise<Record<string, unknown>> {
    ...
    try {
      ...
      if (!workspaceRoot) {
        return {
          content: [{
            type: "text",
            text: "Could not find workspace root. Please ensure the path contains a deno.json or deno.jsonc file."
          }]
        };
      }
      ...
      let output = `Deno format ${check ? "check" : "execution"} completed with code: ${result.code}\n\n`;
  ...
  private async handleDenoLint(args: ToolArgs): Promise<Record<string, unknown>> {
  ...
  private async handleDenoCheck(args: ToolArgs): Promise<Record<string, unknown>> {
  ...
  private async handleDenoTest(args: ToolArgs): Promise<Record<string, unknown>> {
  ...
  private async handleDenoRun(args: ToolArgs): Promise<Record<string, unknown>> {
    ...
    try {
      ...
      if (watch) {
        return {
          content: [{
            type: "text",
            text: `Starting Deno run with watch mode: deno ${denoArgs.join(" ")}\n\nNote: This will start a background process. You can stop it using Ctrl+C in the terminal where it's running.`
          }]
        };
      }
      ...
      return {
        content: [{
          type: "text",
          text: `Error running deno script: ${error instanceof Error ? error.message : String(error)}`
        }]
      };
    }
  }

  private async handleDenoInfo(args: ToolArgs): Promise<Record<string, unknown>> {
